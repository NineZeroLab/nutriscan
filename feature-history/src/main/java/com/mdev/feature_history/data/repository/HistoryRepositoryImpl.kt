...
internal class HistoryRepositoryImpl @Inject constructor(
    ...
    override suspend fun getSearchHistory(): Flow<List<SearchHistoryItemDto>> {
        return firebaseRepository.getSearchHistory().map { searchHistoryList ->
            searchHistoryList.map { productDetailsDto ->
                productDetailsDto.toSearchHistoryDto()
            }
        }
    }
}